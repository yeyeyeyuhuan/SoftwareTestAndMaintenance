...
class SPOT:
    ...
    def initialize(self, level=0.98, verbose=True):
        ...
        idx = min(max(idx, 0), n_init - 1)
        self.init_threshold = S[idx]  # t is fixed for the whole algorithm
        ...
        if self.Nt == 0:
            for lvl in [0.95, 0.9, 0.85, 0.8, 0.75, 0.7]:
                idx = int(lvl * n_init)
                idx = min(max(idx, 0), n_init - 1)
    ...
    def _MOM(self):  # Method of Moments
        Yi = self.peaks
        if Yi.size < 2:
            avg = float(np.mean(Yi)) if Yi.size else 0.0
            sigma = max(avg, 1e-12)
            gamma = 0.0
            return gamma, sigma, 0
        avg = np.mean(Yi)
        var = np.var(Yi, ddof=0)
        if var <= 1e-12:
            sigma = max(float(avg), 1e-12)
            gamma = 0.0
            return gamma, sigma, 0
